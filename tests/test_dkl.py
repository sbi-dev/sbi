...
def test_dkl_one_dim_gauss():
    """
    Test whether for two 1D Gaussians the Monte-Carlo-based D-KL gives similar results as the torch implementation.
    """
    ...
    assert (
        torch.abs(torch_dkl - monte_carlo_dkl) < max_dkl_diff
    ), f"Monte Carlo based DKL={monte_carlo_dkl} is too far off from the torch implementation={torch_dkl}."


def test_dkl_multi_dim_gauss():
    """
    Test whether for two 2D Gaussians the Monte-Carlo-based D-KL gives similar results as the torch implementation.
    """
