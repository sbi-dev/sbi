...
def test_infer_tensor_device_returns_none_without_tensors():
    """A graph without tensors or module parameters yields no device."""
    assert torchutils.infer_tensor_device(5) is None
    assert torchutils.infer_tensor_device(None) is None
    assert torchutils.infer_tensor_device(object()) is None
    assert torchutils.infer_tensor_device({"a": [1, "two"]}) is None


def test_infer_tensor_device_finds_module_parameters():
    """Module parameters and buffers are the strongest signal."""
    module = torch.nn.Linear(2, 2)
    assert torchutils.infer_tensor_device(module) == "cpu"
    ...
    assert torchutils.infer_tensor_device(BufferOnly()) == "cpu"

    class EmptyModule(torch.nn.Module):
        pass

    assert torchutils.infer_tensor_device(EmptyModule()) is None


def test_infer_tensor_device_walks_containers_and_attributes():
    """Tensors are found inside dicts, lists, tuples, and plain objects."""
    tensor = torch.zeros(2)
    assert torchutils.infer_tensor_device([tensor]) == "cpu"
    assert torchutils.infer_tensor_device({"nested": (None, tensor)}) == "cpu"
    ...
    holder = Holder()
    holder.t = tensor
    assert torchutils.infer_tensor_device(holder) == "cpu"

    assert torchutils.infer_tensor_device(torch.zeros(2)) == "cpu"


def test_infer_tensor_device_handles_cycles():
    """A cyclic object graph must not recurse forever."""
    ...
    root = Holder()
    root.self = root
    root.tensor = torch.zeros(2)
    assert torchutils.infer_tensor_device(root) == "cpu"
