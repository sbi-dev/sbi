...
def run(
    ...
    max_num_epochs_ll=100,
    max_num_steps_post=100,
...
"""
    ...
    for r in range(num_rounds):
        ...
        if r == num_rounds - 1:
            max_num_epochs_ll = 10000
            max_num_steps_post = 2000
        ...
        density_estimator = inference_method.append_simulations(
            ...
            max_num_epochs=max_num_epochs_ll,
        )
        ...
        posterior.train(
            ...
            max_num_steps=max_num_steps_post,
        )
        posteriors.append(posterior)
        alpha = alpha * alpha_decay
        cat = torch.distributions.Categorical(probs=torch.tensor([alpha, 1 - alpha]))
        mix = Mixture(cat, [prior, posterior])
        proposal = mix
...
def benchmark_single(
    ...
    for i, num_sim in enumerate(num_simulations):
        samples, num_sim, _, _ = run(
            ...
            num_sim,
            num_observation=num_observation,
            num_rounds=num_rounds[i],
            flow_paras=flow_paras,
        )
        df = evaluate_metric(task, samples, num_sim, num_observation, "SNLEP")
