...
class MixedDensityEstimator(nn.Module):
    ...
    def log_prob(self, x: Tensor, context: Tensor) -> Tensor:
        ...
        input = torch.log(cont_x) if self.log_transform_x else cont_x
        condition = reshape_to_iid_batch_event(condition, event_shape=condition.shape[1:])
        input_de = reshape_to_iid_batch_event(input, input.shape[1:])
