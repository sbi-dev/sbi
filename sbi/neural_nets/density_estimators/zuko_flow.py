...
class FlowMatchingEstimator(ConditionalDensityEstimator):
    def __init__(
        ...
        embedding_net_input: nn.Module = nn.Identity(),
        embedding_net_condition: nn.Module = nn.Identity(),
        num_freqs: int = 3,
    ) -> None:
        ...
        self._embedding_net_input = embedding_net_input
        self._embedding_net_condition = embedding_net_condition

        self.register_buffer("freqs", torch.arange(1, num_freqs + 1) * Tensor(math.pi))
        self.register_buffer('zeros', torch.zeros(input_shape))
        self.register_buffer('ones', torch.ones(input_shape))

    @property
    def embedding_net_x(self):
        return self._embedding_net_input

    @property
    def embedding_net_y(self):
        return self._embedding_net_condition
    ...
    def forward(self, input: Tensor, condition: Tensor, t: Tensor) -> Tensor:
        ...
        embedded_input = self._embedding_net_input(input)
        embedded_condition = self._embedding_net_condition(condition)

        # broadcast to match shapes of theta, x, and t
        # TODO: Does this comply with SBI shape conventions?
        embedded_input, embedded_condition, t = broadcast(
            embedded_input, embedded_condition, t, ignore=1
        )

        # return the estimated vector field
        return self.net(torch.cat((embedded_input, embedded_condition, t), dim=-1))
    ...
    def loss(self, input: Tensor, condition: Tensor, **kwargs) -> Tensor:
        ...
        return mse_loss(self.forward(theta_prime, condition, t), vector_field)
    ...
    def log_prob(self, input: Tensor, condition: Tensor, **kwargs) -> Tensor:
        ...
        dist = self.flow(condition=condition)
    ...
    def flow(self, condition: Tensor) -> Distribution:
