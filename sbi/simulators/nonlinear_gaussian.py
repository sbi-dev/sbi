...
class NonlinearGaussianSimulator(Simulator):
    ...
    def get_ground_truth_parameters():
        """Return ground truth parameters as specified in 'Sequential Neural Likelihood' paper.
        
        Returns:
            torch.Tensor [parameter_dim] -- Ground truth parameters used to generate an observation.
        """
        return torch.Tensor([-0.7, -2.9, -1.0, -0.9, 0.6])

    def get_ground_truth_observation(self):
        """Return normalized ground truth observation using same seed as 'Sequential Neural Likelihood' paper.
        
        Returns:
            torch.Tensor [observation_dim] -- Ground truth observation generated by ground truth parameters.
        """
        # NOTE: I was not able to reproduce these observations from the above params. bug?
        mean, std = self._get_observation_normalization_parameters()
    ...
    def get_ground_truth(self):
        ...
        Returns:
            (torch.Tensor, torch.Tensor) -- Ground truth parameters and corresponding data.
        """
