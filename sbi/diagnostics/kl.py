...
_NORMALIZED = (Distribution, DirectPosterior, VectorFieldPosterior, VIPosterior)
...
def kl_divergence_mc(
    ...
    $\theta_i \sim p$. Both `p` and `q` must have a normalized `log_prob()`, so
    posteriors that sample with MCMC, rejection or importance sampling are refused.
    ...
    Raises:
        ...
        ValueError: If fewer than two samples are used, or if a sample of `p` is
    ...
    for name, dist in (("p", p), ("q", q)):
        if not isinstance(dist, _NORMALIZED):
            raise NotImplementedError(
                f"`{name}` is a `{type(dist).__name__}`, whose `log_prob()` is not a "
                "normalized density. Use a sample-based metric such as "
                "`sbi.utils.metrics.c2st` instead."
            )
