...
class VPSDE(SDE):
    ...
    def prior_logp(self, z):
        shape = z.shape
        N = np.prod(shape[1:])
        logps = -N / 2.0 * np.log(2 * np.pi) - torch.sum(z ** 2, dim=tuple(range(1,len(shape)))) / 2.0
        return logps
...
class subVPSDE(SDE):
    ...
    def prior_logp(self, z):
        shape = z.shape
        N = np.prod(shape[1:])
        return -N / 2.0 * np.log(2 * np.pi) - torch.sum(z ** 2, dim=tuple(range(1,len(shape)))) / 2.0
...
class VESDE(SDE):
    ...
    def prior_logp(self, z):
        ...
        return -N / 2.0 * np.log(2 * np.pi * self.sigma_max ** 2) - torch.sum(
            z ** 2, dim=tuple(range(1,len(shape)))
        ) / (2 * self.sigma_max ** 2)
