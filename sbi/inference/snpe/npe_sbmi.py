...
from sbi.inference.snpe.npe_sbmi_base import PosteriorEstimatorSBMI
...
class NPE_SBMI(PosteriorEstimatorSBMI):
    def __init__(
        self,
        model_net,
        partition,
        ...
        r"""SBMI [1] based on SNPE-C.

        [1] _Simultaneous identification of models and parameters of scientific
            simulators_,
            Schröder et al., ICML 2024,   https://arxiv.org/abs/2305.15174
        ...
        Args:
            model_net: MoGr or similar net to estimate binary model components.
            ...
            loss_kwargs: Additional keyword arguments to be passed to the loss function.
                for MoGr use "prob", for categorical "log_prob",
                as different quantities are returnd by the model CDE
                (in ["prob", "log_prob"])
        """
    ...
    def train(
        ...
        if self._round > 0:
            raise NotImplementedError(
                "NPE_SBMI currently only supports one round of training."
            )
