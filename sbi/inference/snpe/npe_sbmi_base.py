...
class PosteriorEstimatorSBMI(NeuralInference, ABC):
    def __init__(
        self,
        model_net,
        partition,
        prior: Optional[Distribution] = None,
        density_estimator: Union[str, Callable] = "mdn_flex",
        device: str = "cpu",
        ...
        Args:
            model_net: NN to estimate posterior of binary model components. for ex. MoGr
            partition: list of integers, indicating the number of parameters for each
                model component. The sum of the list should be equal to the number of
        ...
        self._model_net = model_net
        self._partition = partition
        self.model_net_type = loss_kwargs["model_net"]

        # track losses in the summary
        self._summary["sbi_loss"] = []
        self._summary["model_loss"] = []
    ...
    def train(
        ...
        Args:
            ...
            model_loss_weight: factor to multiply model_loss with. hyperparameter for
                trading of model vs parameter loss.
        ...
        self._model_net.to(self._device)

        if not resume_training:
            # add params of MADE (_model_net) and embedding (included in _neural_net)
            self.optimizer = optim.Adam(
                [
                    {"params": self._neural_net.parameters()},
                    {"params": self._model_net.parameters()},
                ],
                lr=learning_rate,
            )
    ...
    def _loss(
        ...
        SBI-loss: loss with proposal correction (`round_>0`) or without it (`round_=0`).

        The loss is the negative log prob. Irrespective of the round,
        it can be weighted with a calibration kernel.
        ...
        # mask: encoded_x[:, -n_model_components:]
        p_m_hat = self._model_net(
            encoded_x[:, -n_model_components:],
            context=encoded_x[:, :-n_model_components],
        )
        ...
        # log_prob.shape: batchsize
        sbi_loss = -(calibration_kernel(x) * log_prob)

        self._summary["sbi_loss"].append(sbi_loss.mean().item())
        self._summary["model_loss"].append(model_loss.mean().item())

        return sbi_loss + model_loss * model_loss_weight
